"""
Core functions and utilities for the Heuristic Adaptive Lake Filter (HALF).
...
Basic validation metrics
  ...
  compute_median_residual             Signed median residual (est - ref), NaN robust.
...
def compute_median_residual(y_est, y_ref):
    """
    Compute median signed residual.

    Residual is defined as:
        y_est - y_ref

    Therefore:
        positive value (bias) = estimate is higher than reference
        negative value (bias) = estimate is lower than reference

    Parameters:
        y_est (numeric array-like): Estimated or comparison values.
        y_ref (numeric array-like): Reference or ground-truth values.
    ...
    mask = ~np.isnan(y_est) & ~np.isnan(y_ref)

    if np.sum(mask) == 0:
        return np.nan

    median_residual = np.median(y_est[mask] - y_ref[mask])
...
def apply_heuristic_thresholds(
    ...
    wse_u_ice_min : float, default 0.1
...
def drop_eval_in_apply_gaps(
    ...
    df_eval : pandas.DataFrame
        The evaluation dataframe to be filtered. Must contain a datetime-like 
        column named by `datetime_col`. May include rows that are not present 
        in `df_apply`. Original row order and index are preserved.
...
def apply_customized_filter(
    ...
    if apply_low_pass_filter == 'no':
        ...
        while (updated_length < initial_length) and (n_while < 40): # All conditions must be satisfied
            ...
            # Remove bad observations flagged in PIC2 and PID0: specular_ringing_bad, xovr_cal_bad, and low_coh_bad.
            ...
            if 'lowess' in filter_type:
                ...
                # If the proportion is high, having a very small minfrac may lead to overfitting.
        ...
        The following section reintroduces selected high-quality observations before round-2 filtering when r2_filter == "yes".
        ...
        if r2_filter == 'yes' and n_while >0: # If this option is turned on, and round-1 is valid.
            ...
            while (updated_length < initial_length) and (n_while_r2 < 5): # A max of 5 iteration times to avoid over-rejection in round-2 filtering.
                ...
                df_eval = df_eval.sort_values('datetime', kind='mergesort').reset_index(drop=True)
