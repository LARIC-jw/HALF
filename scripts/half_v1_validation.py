...
INPUT_SCHEMA = {
    "LakeSP time series": {
        ...
        "notes": (
            ...
            "than time_str. "
            "If SP_retrieval_method is set to on-premise, the local "
            "df_Hydrocron_<version>.csv file will be used. For user convenience, "
            "df_Hydrocron_<version>.csv files for both versions during nominal "
            "cycles 1-32 are provided in the LakeSP_data folder."
        ),
    },
    "Gauge data": {
        ...
        "notes": (
            ...
            "Collected gauge data are provided in the gauge_data folder."
        ),
    },
    "Cycle lookup table": {
        "source": "CSV table mapping SWOT science-orbit cycles to UTC times",
        "default_relative_path": "LakeSP_data/SWOT_cycle_lookup_table.csv",
        "notes": (
            "The SWOT cycle-to-UTC lookup table, covering calibration/validation "
            "cycle 401 through science/nominal cycle 40, is provided in the "
            "LakeSP_data folder."
        ),
    },
}
...
RUN_CONFIG = {
    ...
    "cycle_lookup_csv": "LakeSP_data/SWOT_cycle_lookup_table.csv",
...
if SP_retrieval_method == 'on-premise': # retrieve all lakes from the previously saved 'on-premise' file
    df_Hydrocron = pd.read_csv(os.path.join(work_dir, f"LakeSP_data/df_Hydrocron_{version_filename}.csv"))
...
if SP_retrieval_method == 'Hydrocron':
    os.makedirs(os.path.join(work_dir, "LakeSP_data"), exist_ok=True)
    df_Hydrocron.to_csv(os.path.join(work_dir, "LakeSP_data", f"df_Hydrocron_{version_filename}.csv"), index=False)
