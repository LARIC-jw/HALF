...
HALF aims to remove outliers from SWOT LakeSP water surface elevation (WSE)
time series while preserving hydrologically meaningful temporal dynamics.

The HALF workflow follows three primary steps:
...
This script integrates both LakeSP filtering and gauge validation to reproduce 
the results reported in Trudel et al. (2026, in review).

It retrieves or loads LakeSP time series, reads paired gauge records, 
applies HALF, computes validation metrics, and writes/plots lake-level and 
observation-level outputs.

Script name and version
-------
half_v1_validation.py: 
...
implementation, maintenance, review, testing, and validation of the code.
...
# Import customized functions from half_v1_functions.py
# See half_v1_functions.py for definitions and functionality.
from half_v1_functions import (
...
INPUT_SCHEMA = {
    "LakeSP time series": {
        ...
        "notes": (
            ...
            "than time_str. "
            "By default, LakeSP observations are retrieved directly from the "
            "Hydrocron API. If SP_retrieval_method is set to on-premise, the "
            "user must provide a local df_Hydrocron_<version>.csv file in the "
            "directory specified by LakeSP_data_dir."
        ),
    },
    "Gauge data": {
        ...
        "notes": (
            ...
            "Gauge data should be placed in the gauge_data folder. The gauge-data "
            "compilation is archived separately from the HALF software release. "
...
OUTPUT_SCHEMA = {
    ...
    "validation_lake_stats_csv": (
        "CSV export of the full lake-level validation statistics table, including "
        "the reference-lake keep_mask, written to plots_dir using the filename "
        "df_validation_lake_stats_HALF_<script_version>_"
        "LakeSP_<version_filename>.csv."
    ),
    "plots_dir": (
        "Directory named plots_validation_HALF_<script_version>_"
        "LakeSP_<version_filename> containing per-lake diagnostics and summary "
        "figures."
    ),
}
...
RUN_CONFIG = {
    ...
    # SWOT orbit cycle and UTC time window. The cycle-to-UTC lookup table is
    ...
    # Replace "." with an absolute path for a local run.
    "work_dir": ".",

    # LakeSP retrieval mode:
    #   - "Hydrocron" (default): query the Hydrocron API lake by lake.
    #   - "on-premise": read a previously saved df_Hydrocron_<version>.csv file.
    "SP_retrieval_method": "Hydrocron",    
    # Note: The LakeSP observations used for the validation reported in Trudel et al.
    # (2026, in review) were retrieved from the PO.DAAC Hydrocron API on
    # May 28, 2026 (US Central Time). The retrieval covered available LakeSP_Prior
    # time series for Version C and Version D during science orbit cycles 1-32
    # (2023-07-21 to 2025-05-18). Cached Hydrocron CSV files are not included 
    # in this repository.
    
    # Directory for local Hydrocron LakeSP CSV files.
    # If SP_retrieval_method is set to 'Hydrocron', the retrieved LakeSP 
    # records will be cached here as df_Hydrocron_<version>.csv.
    # If SP_retrieval_method = 'on-premise', the script reads 
    # df_Hydrocron_<version>.csv previously saved in this folder.
    # This value should not be left empty; use "LakeSP_data" by default.
    # Relative paths are interpreted from work_dir.
    "LakeSP_data_dir": "LakeSP_data",

    # Validation script label used in output filenames following the common
    # ..._HALF_<script_version>_LakeSP_<version_filename> naming convention.
    "script_version": "v1",
...
LakeSP_data_dir = RUN_CONFIG["LakeSP_data_dir"]
if not str(LakeSP_data_dir).strip():
    raise ValueError(
        "LakeSP_data_dir is empty. Please enter the directory containing or "
        "saving df_Hydrocron_<version>.csv, e.g., 'LakeSP_data'."
    )
if not os.path.isabs(LakeSP_data_dir):
    LakeSP_data_dir = os.path.join(work_dir, LakeSP_data_dir)
...
plots_dir = os.path.join(
    work_dir,
    f"plots_validation_HALF_{script_version}_LakeSP_{version_filename}",
)
...
df_Canada_GNSS = pd.read_csv(gauge_data_path, sep=',', encoding='iso-8859-2')
unique_pld_ids = df_Canada_GNSS["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs.
this_test_lakes = pd.DataFrame(columns=['lake_id', 'gauge_source', 'gauge_dir', 'gauge_id', 'gauge_datum'])
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
...
# Canada_no_GNSS
gauge_data_path = work_dir+'/gauge_data/Canada/Canada_in_situ_no_GNSS.csv'
df_Canada_no_GNSS = pd.read_csv(gauge_data_path, sep=',', encoding='iso-8859-2')
unique_pld_ids = df_Canada_no_GNSS["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs.
this_test_lakes = pd.DataFrame(columns=['lake_id', 'gauge_source', 'gauge_dir', 'gauge_id', 'gauge_datum'])
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'Canada_no_GNSS',
        'gauge_dir': gauge_data_path,
        'gauge_id': 'Canada_no_GNSS', #String
        'gauge_datum': np.nan
    }])
    this_test_lakes = pd.concat([this_test_lakes, new_row], ignore_index=True)
    test_lakes = pd.concat([test_lakes, new_row], ignore_index=True)
...
- 'medianE' is computed with 'compute_median_residual(swot, gauge)', which
  follows: residual = first argument - second argument.
"""
...
if SP_retrieval_method == "on-premise":
    # Retrieve all lakes from a previously saved local Hydrocron/LakeSP file.
    hydrocron_csv = os.path.join(
        LakeSP_data_dir,
        f"df_Hydrocron_{version_filename}.csv"
    )
    if not os.path.exists(hydrocron_csv):
        raise FileNotFoundError(
            f"Local LakeSP file not found: {hydrocron_csv}\n"
            "For on-premise mode, please place df_Hydrocron_<version>.csv "
            "in LakeSP_data_dir, or set SP_retrieval_method = 'Hydrocron' "
            "to retrieve LakeSP observations directly from the Hydrocron API."
        )
    df_Hydrocron = pd.read_csv(hydrocron_csv)
elif SP_retrieval_method == "Hydrocron":
    # Initialize an empty dataframe for Hydrocron retrieval.
    # Records will be appended lake by lake inside the processing loop.
    df_Hydrocron = pd.DataFrame()
else:
    raise ValueError(
        "Unsupported SP_retrieval_method. "
        "Use either 'Hydrocron' or 'on-premise'."
    )
...
for feature_id in test_lakeIDs: # test_lakeIDs contain a list of unique PLD lake IDs.
    ...
    if gauge_source == 'Canada_GNSS': # Lakes in Canada
        gauge_df = df_Canada_GNSS.loc[df_Canada_GNSS["PLD_Lake_ID"] == feature_id].copy()
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime'])
        gauge_df = gauge_df.sort_values('gauge_datetime')
        time_tolerance = '24h'

    if gauge_source == 'Canada_no_GNSS': # Lakes in Canada
        gauge_df = df_Canada_no_GNSS.loc[df_Canada_no_GNSS["PLD_Lake_ID"] == feature_id].copy()
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime'])
        gauge_df = gauge_df.sort_values('gauge_datetime')
        time_tolerance = '24h'
    ...
    # Save the plot.
    plt.savefig(os.path.join(plots_dir, f'Attempt_{filter_attempt}_lakeID_{feature_id}_{filter_type}_HALF_{script_version}_LakeSP_{version_filename}.png'),bbox_inches='tight')
    if show_filtering_evolution == 'no':
        plt.close()  # free up memory

# Save Hydrocron time series into local disk
if SP_retrieval_method == "Hydrocron":
    os.makedirs(LakeSP_data_dir, exist_ok=True)
    df_Hydrocron.to_csv(os.path.join(LakeSP_data_dir, f"df_Hydrocron_{version_filename}.csv"), index=False)
...
# We remove the fixed union of invalid lakes identified from both Version C and Version D
# to ensure that validation summaries are based on the same lake set across versions.
print("Gauge lake count:", len(df_lake_stats_intact))
# Invalid lakes from both versions include the following:
...
out_png = os.path.join(
    plots_dir,
    f"global_lake_map_GCS_countries_lakes_area_HALF_{script_version}_LakeSP_{version_filename}.png"
)
out_pdf = os.path.join(
    plots_dir,
    f"global_lake_map_GCS_countries_lakes_area_HALF_{script_version}_LakeSP_{version_filename}.pdf"
)
...
df_lake_stats_valid_backup.to_csv(
    os.path.join(plots_dir, f"df_validation_lake_stats_HALF_{script_version}_LakeSP_{version_filename}.csv"),
    index=False,
)
...
if len(lakes_for_validation) > 0:
    print(str(len(subset_cross_pass_corrected) / len(lakes_for_validation) * 100.0) + "%")
else:
    print("No validated lakes available for cross-pass correction percentage.")
...
def plot_hist_overlay_4groups(df_lake_stats_valid_backup,
    ...
    if outname is None:
        outname_base = f"Lake_validation_hist_HALF_{script_version}_LakeSP_{version_filename}"
    else:
...
if lakes_for_validation:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'Lake_validation_stats_HALF_{script_version}_LakeSP_{version_filename}.png'), bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, f'Lake_validation_stats_HALF_{script_version}_LakeSP_{version_filename}.pdf'), bbox_inches='tight')
    # plt.close(fig)
    ...
    # available from the SWOT events/cycle table distributed by PO.DAAC/JPL:
    # https://podaac.jpl.nasa.gov/SWOT-events/SWOT_events.html#cycletabletop.
    # Here, the table is manually entered only for plot labeling.    
    SWOT_CYCLE_START_UTC = {
        1: "2023-07-21T05:33:45Z",
        2: "2023-08-11T02:18:53Z",
        3: "2023-08-31T23:03:58Z",
        4: "2023-09-21T19:49:03Z",
        5: "2023-10-12T16:34:05Z",
        6: "2023-11-02T13:19:11Z",
        7: "2023-11-23T10:04:16Z",
        8: "2023-12-14T06:49:20Z",
        9: "2024-01-04T03:34:26Z",
        10: "2024-01-25T00:19:32Z",
        11: "2024-02-14T21:04:37Z",
        12: "2024-03-06T17:49:41Z",
        13: "2024-03-27T14:34:44Z",
        14: "2024-04-17T11:19:50Z",
        15: "2024-05-08T08:04:54Z",
        16: "2024-05-29T04:49:59Z",
        17: "2024-06-19T01:35:04Z",
        18: "2024-07-09T22:20:07Z",
        19: "2024-07-30T19:05:12Z",
        20: "2024-08-20T15:50:16Z",
        21: "2024-09-10T12:35:22Z",
        22: "2024-10-01T09:20:30Z",
        23: "2024-10-22T06:05:34Z",
        24: "2024-11-12T02:50:38Z",
        25: "2024-12-02T23:35:43Z",
        26: "2024-12-23T20:20:47Z",
        27: "2025-01-13T17:05:51Z",
        28: "2025-02-03T13:50:55Z",
        29: "2025-02-24T10:36:01Z",
        30: "2025-03-17T07:21:05Z",
        31: "2025-04-07T04:06:09Z",
        32: "2025-04-28T00:51:15Z",
        33: "2025-05-18T21:36:19Z",
        34: "2025-06-08T18:21:24Z",
        35: "2025-06-29T15:06:30Z",
        36: "2025-07-20T11:51:36Z"
    }    
    cyc_local = pd.DataFrame({
        "Cycle": list(SWOT_CYCLE_START_UTC.keys()),
        "Start Time (UTC)": list(SWOT_CYCLE_START_UTC.values()),
    })
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'Lake_retention_box3_by_cycle_HALF_{script_version}_LakeSP_{version_filename}.png'),
                bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, f'Lake_retention_box3_by_cycle_HALF_{script_version}_LakeSP_{version_filename}.pdf'),
                bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'MAE_box_icefree_vs_ice_HALF_{script_version}_LakeSP_{version_filename}.png'),
                bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, f'MAE_box_icefree_vs_ice_HALF_{script_version}_LakeSP_{version_filename}.pdf'),
                bbox_inches='tight')
    #plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'RelErr_box_icefree_vs_ice_HALF_{script_version}_LakeSP_{version_filename}.png'),
                bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, f'RelErr_box_icefree_vs_ice_HALF_{script_version}_LakeSP_{version_filename}.pdf'),
                bbox_inches='tight')
    #plt.close(fig)
    ...
    if all(c in dfv.columns for c in needed):
        ...
        if len(areas) >= 5:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(plots_dir, f'RelErrVar_box_by_area_HALF_{script_version}_LakeSP_{version_filename}.png'),
                        bbox_inches='tight')
            plt.savefig(os.path.join(plots_dir, f'RelErrVar_box_by_area_HALF_{script_version}_LakeSP_{version_filename}.pdf'),
                        bbox_inches='tight')
            #plt.close(fig)
    ...
    def plot_global_mae_maps(df_err, hc_ll, plots_dir, script_version, version_filename):
        ...
        out_path = os.path.join(
            plots_dir,
            f"Global_MAE_maps_HALF_{script_version}_LakeSP_{version_filename}.pdf"
        )
    ...
    def plot_mae_cdfs(
        ...
        out_name = f"Lake_MAE_CDFs_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
    ...
    def plot_global_medianE_maps(df_err, hc_ll, plots_dir, script_version, version_filename):
        ...
        out_path = os.path.join(
            plots_dir,
            f"Global_medianE_maps_HALF_{script_version}_LakeSP_{version_filename}.pdf"
        )
    ...
    def plot_medianE_cdfs(
        ...
        out_name = f"Lake_medianE_CDFs_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        out_path = os.path.join(plots_dir, out_name)
    ...
    def plot_global_correlation_map(
        ...
        out_path = os.path.join(
            plots_dir,
            f"Global_correlation_map_HALF_{script_version}_LakeSP_{version_filename}.pdf"
        )
    ...
    def plot_correlation_cdf(
        ...
        out_name = f"Lake_correlation_CDF_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        out_path = os.path.join(plots_dir, out_name)
    ...
    def plot_global_retention_map(
        ...
        out_path = os.path.join(
            plots_dir,
            f"Global_retention_map_HALF_{script_version}_LakeSP_{version_filename}.pdf"
        )
    ...
    def plot_retention_cdf(
        ...
        out_name = f"Lake_retention_CDF_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        out_path = os.path.join(plots_dir, out_name)
    ...
    def plot_normalized_variability_error_map(
        ...
        out_path = os.path.join(
            plots_dir,
            f"Global_norm_var_error_map_{cfg['file_tag']}_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        )
        plt.savefig(out_path, dpi=dpi, bbox_inches="tight")
        plt.show()
    ...
    def plot_variability_scatter_and_normalized_error_cdf(
        ...
        out_path = os.path.join(
            plots_dir,
            f"Lake_variability_scatter_and_norm_error_CDF_{cfg['file_tag']}_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        )
        plt.savefig(out_path, dpi=dpi, bbox_inches="tight")
        plt.show()
    ...
    def plot_norm_var_error_by_area_bins(
        ...
        out_path = os.path.join(
            plots_dir,
            f"NormVarError_box_by_area_{cfg['file_tag']}_HALF_{script_version}_LakeSP_{version_filename}.{save_format}"
        )
        plt.savefig(out_path, dpi=dpi, bbox_inches="tight")
        plt.show()
