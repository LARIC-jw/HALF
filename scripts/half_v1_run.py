...
# Import reusable HALF functions from half_v1_functions.py.
...
OUTPUT_SCHEMA = {
    ...
    "df_Hydrocron_output_csv": (
        "CSV export of df_Hydrocron written to work_dir using the "
        "filename df_Hydrocron_HALF_{script_version}_LakeSP_{version_filename}.csv. "
        "This file contains the original Hydrocron/LakeSP observations augmented " 
        "with HALF outputs, including filter_flag, wse_adjusted, filter_attempt, "
        "and optional observation-level threshold details."
    ),
    "threshold_summary_csv": (
        "CSV export of df_threshold_summary written to work_dir using the "
        "filename df_threshold_summary_HALF_<script_version>_"
...
df_Hydrocron_output_csv = os.path.join(
    work_dir,
    f"df_Hydrocron_HALF_{script_version}_LakeSP_{version_filename}.csv",
)

# Combined final-attempt threshold-summary output file.
threshold_summary_csv = os.path.join(
    work_dir,
    f"df_threshold_summary_HALF_{script_version}_LakeSP_{version_filename}.csv",
)
...
1. df_Hydrocron
   ...
   - intra_cycle_flag:
    ...
    See apply_heuristic_thresholds() in half_v1_functions.py for the full 
...
for feature_id in lake_ids:
    ...
    # See detailed parameter definitions in calibrate_heuristic_thresholds() in half_v1_functions.py.
    ...
    # See detailed parameter definitions in apply_customized_filter() in half_v1_functions.py.
    ...
    if execute_intra_cycle_adjustment == 'no':
        ...
        # See detailed parameter definitions in sp_cycle_adjustment() in half_v1_functions.py.
    ...
    plt.savefig(
        os.path.join(
            work_dir,
            f'Attempt_{filter_attempt}_lakeID_{feature_id}_{filter_type}_HALF_{script_version}_LakeSP_{version_filename}.png'
        ),
        bbox_inches='tight'
    )
