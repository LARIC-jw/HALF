"""
Customized LakeSP WSE filter (v10)
...
end_time = "2025-11-21T00:00:00Z"  # two year of data
#Ref: cycle-to-UTC look-up table: https://podaac.jpl.nasa.gov/SWOT-events/SWOT_events.html#cycletabletop
start_cycle = 1
end_cycle = 39 #40

work_dir = r'D:\D\Research\Projects\SWOT\Initial_global_lakes\Codes\Updated_codes_for_processing_LakeSP\Melanie_Filter_v10'
...
Region       	  lake-count   Frequency	    Sources	                       Data-providers
  US              264	       Hourly           USBR, USGS	                   Merritt Harlan
  Canada GNSS	  217	       Hourly 	        ECCC, CEHQ, Spence, HQ, UDES   Mélanie Trudel
  Canada UN	      65	       Hourly 	        ECCC, CEHQ                     Mélanie Trudel	
  ...
  Switzerland     25           Hourly           Bafu                           CNES
  Norway          232          Hourly           NVE                            CNES
...
"""
# Old version, now average hourly to increase speed
...
df_Bafu = pd.read_csv(work_dir+'/gauge_data/Europe-from-cnes/bafu_in_situ.csv', sep=',', encoding='iso-8859-2')                 
unique_pld_ids = df_Bafu["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs. 
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'Bafu',
        'gauge_dir': work_dir+'/gauge_data/Europe-from-cnes/bafu_in_situ.csv',
...
# NVE gauge (Norway)
df_nve = pd.read_csv(work_dir+'/gauge_data/Europe-from-cnes/nve_in_situ.csv', sep=',', encoding='iso-8859-2')                 
unique_pld_ids = df_nve["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs. 
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'nve',
        'gauge_dir': work_dir+'/gauge_data/Europe-from-cnes/nve_in_situ.csv',
        'gauge_id': 'nve', #String
        'gauge_datum': np.nan
    }])
    test_lakes = pd.concat([test_lakes, new_row], ignore_index=True) 
...
# US
df_US = pd.read_csv(work_dir+'/gauge_data/NA-from-Merritt-Harlan/US_in_situ.csv', sep=',', encoding='iso-8859-2')                 
unique_pld_ids = df_US["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs. 
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'US',
        'gauge_dir': work_dir+'/gauge_data/NA-from-Merritt-Harlan/US_in_situ.csv',
        'gauge_id': 'US', #String
        'gauge_datum': np.nan
    }])
    test_lakes = pd.concat([test_lakes, new_row], ignore_index=True) 
...
# Canada_GNSS
df_Canada = pd.read_csv(work_dir+'/gauge_data/Canada-from-Melanie-Trudel/Canada_in_situ_gnss.csv', sep=',', encoding='iso-8859-2')                 
unique_pld_ids = df_Canada["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs. 
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'Canada_GNSS',
        'gauge_dir': work_dir+'/gauge_data/Canada-from-Melanie-Trudel/Canada_in_situ_gnss.csv',
...
# Canada_NO GNSS
df_Canada_UN = pd.read_csv(work_dir+'/gauge_data/Canada-from-Melanie-Trudel/Canada_in_situ.csv', sep=',', encoding='iso-8859-2')                 
unique_pld_ids = df_Canada_UN["PLD_Lake_ID"].unique() # Retrieve unique lake (PLD) IDs. 
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        'lake_id': unique_pld_ids[n], #integer
        'gauge_source': 'Canada_UN',
        'gauge_dir': work_dir+'/gauge_data/Canada-from-Melanie-Trudel/Canada_in_situ.csv',
...
lakes_to_exclude = [4530045882, 4530047122, 4530055932, 4530182732, 4530360803, 4530368712, 4530388743,  4530389213, \
                    6420530103, 7421035423, 7520006182, 7520021732, 7120849393,2320145123,2320148843,2140016762,2320145652, \
...
#test_lakeIDs = [2510066302, 2510165942, 2510207473, 2510219872, 2510280033, 2510280312, 7421019582, 7520022373, 7310000692, 7421019143, \
#                7421086652, 2510163762, 2510164362, 2510165803, 2510193482, 2510270043]
...
print('total numnber of unique PLD lakes with gauge data: ' + str(len(test_lakeIDs))) #1205
...
for feature_id in test_lakeIDs: # test_lakeIDs contain a list of unique PLD lake IDs. 
    ...
    if gauge_source == 'Other_Brazil': # Other reservoirs in Brazil
        ...
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side.
        time_tolerance = '24h'
    if gauge_source == 'Bafu': # Lake in Switzerland
        gauge_df = df_Bafu #pd.read_csv(gauge_dir) #to speed up
        gauge_df = gauge_df[gauge_df["PLD_Lake_ID"] == feature_id] # Filter for PLD_Lake_ID based on the current feature_id
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime']) # Ensure datetime is in datetime64 format.
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side. 
        time_tolerance = '24h'  
    if gauge_source == 'nve': # Lake in Norway
        gauge_df = df_nve #pd.read_csv(gauge_dir) #to speed up
        gauge_df = gauge_df[gauge_df["PLD_Lake_ID"] == feature_id] # Filter for PLD_Lake_ID based on the current feature_id
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime']) # Ensure datetime is in datetime64 format.
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side. 
        time_tolerance = '24h'  
    if gauge_source == 'Canada_GNSS': # Lake in Canada
        gauge_df = df_Canada #pd.read_csv(gauge_dir) #to speed up
        gauge_df = gauge_df[gauge_df["PLD_Lake_ID"] == feature_id] # Filter for PLD_Lake_ID based on the current feature_id
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime']) # Ensure datetime is in datetime64 format.
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side. 
        time_tolerance = '24h'          
    if gauge_source == 'Canada_UN': # Lake in Canada
        gauge_df = df_Canada_UN #pd.read_csv(gauge_dir) #to speed up
        gauge_df = gauge_df[gauge_df["PLD_Lake_ID"] == feature_id] # Filter for PLD_Lake_ID based on the current feature_id
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime']) # Ensure datetime is in datetime64 format.
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side. 
        time_tolerance = '24h'  
    if gauge_source == 'US': # Lake in US
        gauge_df = df_US #pd.read_csv(gauge_dir) #to speed up
        gauge_df = gauge_df[gauge_df["PLD_Lake_ID"] == feature_id] # Filter for PLD_Lake_ID based on the current feature_id
        gauge_df['gauge_datetime'] = pd.to_datetime(gauge_df['gauge_datetime']) # Ensure datetime is in datetime64 format.
        gauge_df = gauge_df.sort_values('gauge_datetime') # Sort gauge_df by time, to be on the safe side. 
        time_tolerance = '24h'  
    ...
    #plt.plot(gauge_df.gauge_datetime, gauge_df.gauge_wse, marker='o', linestyle='--', markersize=2)
    #plt.title('Lake ID ' + str(feature_id))
    #plt.show()     
    # Remove these individual errors from the gauge record:
    # In addition, any major cap in the gauge data (e.g., 3+ months) during freeze-up period will be excluded
    if gauge_source is None: 
        ...
        if feature_id == 7421019143: # double check this. There may still be errors
    ...
    if SP_retrieval_method == 'Hydrocron': # from Hydrocron directly. 
        ...
        collection_name = "SWOT_L2_HR_LakeSP_2.0"  # version C
        enquiry_input =  "https://soto.podaac.earthdatacloud.nasa.gov/hydrocron/v1/timeseries?feature=" + \
                        feature + "&feature_id=" + str(feature_id) + "&start_time=" + start_time + "&end_time=" + end_time + "&output=" + output + "&fields=" + fields + "&collection_name=" + collection_name
        ...
        collection_name = "SWOT_L2_HR_LakeSP_D"  # version D
        enquiry_input =  "https://soto.podaac.earthdatacloud.nasa.gov/hydrocron/v1/timeseries?feature=" + \
                        feature + "&feature_id=" + str(feature_id) + "&start_time=" + start_time + "&end_time=" + end_time + "&output=" + output + "&fields=" + fields + "&collection_name=" + collection_name
        hydrocron_response = requests.get(enquiry_input).json()
    ...
    # if on-premise: df will be retreived from the file previously saved in the local disk;
    # otherwise (if Hydrocron), retrieve from the accumulatively appended df_Hydrocron
    ...
    ## If we only want to evaluate version C
    #df = df[df["collection_shortname"] == "SWOT_L2_HR_LakeSP_2.0"]
    ##---
    ...
    Logic: 
        ...
        - 2. From these selected observations, we calibrate heuristic thresholds for the following key metrics (determined by random forest test):
    ...
    #     quality_f == 1 and crid is "PIC2" or "PID0" or "PGD0".  
    conservative_SQL = (
        '(xovr_cal_q < 1) & ('
        '  (quality_f < 1) '
...
Recall: major outputs from sections above include:
    df_lake_time_series
    df_lake_stats (later replaced by df_lake_stats_intact)
"""
...
df_lake_stats_intact = df_lake_stats.copy() # Make a duplicate
select_rule = 'strict and part of lenient' #'strict only' #'strict and part of lenient' ------- EDIT THIS FOR VALIDATION PREFERENCE
if select_rule == 'strict only':
    # ---- only keep 1_strict
    keep_mask = df_lake_stats_intact['filter_attempt'].isin(['1_strict']) # Just include 1_strict only for now. 
else:
    # ---- also keep part of 2_lenient:
    df_select = df_lake_time_series.copy()
    df_select = df_select.sort_values(['lake_id', 'datetime'])
    
    g = df_select.groupby('lake_id', sort=False)
    
    # Per-lake metrics (indices are lake_ids)
    famode       = g['filter_attempt'].first().str.lower().str.strip()
    span_days    = (g['datetime'].max() - g['datetime'].min()).dt.days
    max_gap_days = g['datetime'].apply(lambda s: s.diff().max() / pd.Timedelta(days=1))
    
    # Keep rules
    keep_strict  = famode[famode.eq('1_strict')].index
    keep_lenient = famode[famode.eq('2_lenient')].index
    ok_gap  = max_gap_days[max_gap_days.fillna(np.inf) <= 120].index
    ok_span = span_days[span_days.fillna(0) >= 365].index
    
    keep_ids = keep_strict.union(keep_lenient.intersection(ok_gap).intersection(ok_span))
    
    # Final: keep rows for qualifying lakes (result stays in df_lake_stats_intact)
    keep_mask = df_lake_stats_intact['lake_id'].isin(keep_ids)
    
# Add a new column keep_mask (1/0) to indicate which lakes are kept for validation
df_lake_stats_intact['keep_mask'] = keep_mask.astype('int8')  # Boolean to 1/0
df_lake_stats_intact.to_csv(work_dir+'/validation_lake_stats.csv', index=False) # Save the result.

# Subset kept lakes for validation
df_lake_stats = df_lake_stats_intact[keep_mask].copy()  
proportion_kept_lakes= (len(df_lake_stats) / len(df_lake_stats_intact))*100
print('Proportion (%) of lakes kept: ' + str(proportion_kept_lakes))


# ---- Plot lake histogram
def plot_hist_intact(df_lake_stats_intact, col="lake_area", bins_per_decade=10):
    """Plot histogram of lake_area with logarithmic bins for df_lake_stats_intact."""
    data = df_lake_stats_intact[col].dropna().astype(float)
    data = data[data > 0]  # exclude zeros for log scale

    # Log-spaced bins
    min_val, max_val = data.min(), data.max()
    bins = np.logspace(np.log10(min_val), np.log10(max_val), 
                       int(np.log10(max_val/min_val)) * bins_per_decade)

    plt.figure(figsize=(8, 6))
    plt.hist(data, bins=bins, edgecolor="black", color="skyblue", alpha=0.6) 
    plt.xscale("log")
    #plt.yscale("log")   # comment this line out if you want only x-axis log
    plt.xlabel("Lake area (km²)")
    plt.ylabel("Number of validated lakes")    
    #plt.grid(True, which="both", linestyle="--", alpha=0.6)
    plt.tight_layout()
    #plt.show()
    plt.savefig(work_dir+'\Plots\Lake_validation_hist'+'_'+script_version+'.png')   
    
def plot_hist_overlay(df_lake_stats_intact, df_lake_stats, 
                      col="lake_area", bins_per_decade=10):
    """Overlay histogram: intact as filled bars, subset as red line, log bins."""
    data_intact = df_lake_stats_intact[col].dropna().astype(float)
    data_subset = df_lake_stats[col].dropna().astype(float)
    data_intact, data_subset = data_intact[data_intact > 0], data_subset[data_subset > 0]

    min_val = min(data_intact.min(), data_subset.min())
    max_val = max(data_intact.max(), data_subset.max())
    ...
    # Base histogram (intact, filled)
    plt.hist(data_intact, bins=bins, edgecolor="black", 
             color="skyblue", alpha=0.6, label="All lakes")

    # Overlay (subset, red step line)
    plt.hist(data_subset, bins=bins, histtype="step", 
             color="red", linewidth=2, label="Filtered lakes")

    plt.xscale("log")
    # plt.yscale("log")   # uncomment if you also want log y-axis
    plt.xlabel("Lake area (km²)")
    plt.ylabel("Number of validated lakes")
    #plt.title("Overlay of lake_area distributions (log bins)")
    plt.legend()
    #plt.grid(True, which="both", linestyle="--", alpha=0.6)
    plt.tight_layout()
    #plt.show()
    plt.savefig(work_dir+'\Plots\Lake_validation_hist_2'+'_'+script_version+'.png')   
plt.rcParams["font.family"] = "Arial"
plt.rcParams.update({
"font.family": "Arial",
...
"figure.titlesize": 22   # suptitle (if you add one)
})   
plot_hist_intact(df_lake_stats_intact, col="lake_area", bins_per_decade=10)
plot_hist_overlay(df_lake_stats_intact, df_lake_stats, col="lake_area", bins_per_decade=10)
# ----
...
if unique_lakes: # If there is at least one lake with gauge data (i.e., rmse is not NaN)
    ...
    for lake_id in unique_lakes: #Loop through each lake
        ...
        #test
        #if np.nanmin(gauge_anom.tolist()) < -10000:
        #    print(lake_id) 
    ...
    axes[0].scatter(gauge_wse_anomalies, SWOT_wse_anomalies, color='r', \
                    label='heuristic filter', s=50, linewidth=0, alpha=0.2) # filtered result
    ...
    #axes[0].set_xlim(-54.43, 43.40) 
    #axes[0].set_ylim(-54.43, 43.40)     
    axes[0].set_xscale('linear')
    axes[0].set_yscale('linear')
    axes[0].set_xlabel('Gauge WSE (m)')
    axes[0].set_ylabel('SWOT WSE (m)')
    axes[0].set_title('Lake WSE (w.r.t. each lake median level)')
    ...
    axes[1].scatter(subset.var_gauge_daily, subset.var_swot_daily, color='r', \
                    label='heuristic filter', s=50, linewidth=0, alpha=0.4) # filtered result
    # Add 1:1 diagonal line
    ...
    #max_val = 230.50
    #test
    #lake_ids_var = subset.loc[subset["var_gauge_daily"] >50, "lake_id"]
    ...
    axes[1].set_ylabel('SWOT WSE variability (m)')
    axes[1].set_title('Seasonal variability (std.)')
    axes[1].grid(True, which='both', linestyle='--', linewidth=0.5)
    axes[1].legend() 
    
    print('RMSE')
    print(np.nanpercentile(subset.rmse, 68))
    print(np.nanpercentile(subset.rmse, 50))
       
    print('relative error of seasonal variability')
    ...
    #For inferring uncertainty only
    #relative_var_error = (subset.var_swot_daily - subset.var_gauge_daily)/subset.var_gauge_daily
    ...
    axes[2].plot(correlation_filtered_sorted, cdf_correlation_filtered, label='heuristic filter', color='red', alpha=0.8)
    # Axis labels and scaling
    axes[2].set_xlim(-1, 1) 
    axes[2].set_ylim(0, 1) 
    ...
    axes[2].set_title('CDF of correlation')
    axes[2].grid(True, which='both', linestyle='--', linewidth=0.5)
    axes[2].legend() 
    
    print('correlation')
    print(correlation_filtered_sorted[np.argmin(np.abs(cdf_correlation_filtered - 0.68))])
    print(correlation_filtered_sorted[np.argmin(np.abs(cdf_correlation_filtered - 0.50))])
    ...
    axes[3].plot(retention_rate_sorted, cdf_retention_rate, label='heuristic filter', color='red', alpha=0.8)
    # Axis labels and scaling
    axes[3].set_xlim(0, 1) 
    axes[3].set_ylim(0, 1) 
    ...
    axes[3].set_title('CDF of retention rates')
    axes[3].grid(True, which='both', linestyle='--', linewidth=0.5)
    axes[3].legend()
    
    print('retention rate')
    print(retention_rate_sorted[np.argmin(np.abs(cdf_retention_rate - 0.68))])
    print(retention_rate_sorted[np.argmin(np.abs(cdf_retention_rate - 0.50))])
    ...
    # ==== Boxplot of per-lake retention rate by SWOT cycle, with date (bottom) & cycle_id (top) ====
    cyc = pd.read_csv(r"D:\D\Research\Projects\SWOT\Initial_global_lakes\Codes\Updated_codes_for_processing_LakeSP\SWOT_cycle_lookup_table.csv")
    
    # standardize column names
    cyc.columns = [c.strip().lower().replace(' ', '_') for c in cyc.columns]
    cid_col = 'cycle'
    start_col = 'start_time_(utc)'    
    
    # ensure types
    cyc[cid_col] = pd.to_numeric(cyc[cid_col], errors='coerce').astype('Int64')
    cyc = cyc.dropna(subset=[cid_col]).copy()
    cyc[cid_col] = cyc[cid_col].astype(int)
    cyc[start_col] = pd.to_datetime(cyc[start_col], errors='coerce')
    
    # --- 2) Figure out cycle range to plot ---
    if 'start_cycle' in locals() and 'end_cycle' in locals():
        sc, ec = int(start_cycle), int(end_cycle)
    else:
        sc, ec = cyc[cid_col].min(), cyc[cid_col].max()
    
    cycles = list(range(sc, ec + 1))
    
    # build date map for the visible range
    cyc_range = cyc[cyc[cid_col].between(sc, ec)].set_index(cid_col)
    date_map = cyc_range[start_col].to_dict()
    
    # --- 3) Compute per-lake, per-cycle retention from df_lake_time_series ---
    # Expect: df_lake_time_series has columns: lake_id, cycle_id, wse, wse_adjusted
    tmp = df_lake_time_series[['lake_id', 'cycle_id', 'wse', 'wse_adjusted']].copy()
    tmp = tmp[tmp['cycle_id'].between(sc, ec)]
    
    # limit to gauge-reference lakes if you kept a list; otherwise use all
    if 'unique_lakes' in locals() and isinstance(unique_lakes, (list, tuple, pd.Series)) and len(unique_lakes) > 0:
        tmp = tmp[tmp['lake_id'].isin(unique_lakes)]
    
    grp = (
        tmp.groupby(['lake_id', 'cycle_id'], as_index=False)
           .agg(n_adj=('wse_adjusted', lambda s: s.notna().sum()),
                n_raw=('wse',          lambda s: s.notna().sum()))
    )
    grp['retention_rate'] = np.where(grp['n_raw'] > 0, grp['n_adj'] / grp['n_raw'], np.nan)
    # (optional) enforce [0,1]
    # grp['retention_rate'] = grp['retention_rate'].clip(0, 1)
    
    # arrange data as list per cycle for the boxplot
    data_per_cycle = [grp.loc[grp['cycle_id'] == c, 'retention_rate'].dropna().values for c in cycles]
    ...
    # subtle fill on boxes
    for box in bp['boxes']:
        box.set_alpha(0.6)
    # dark blue, thicker medians as requested
    for median in bp['medians']:
        median.set_color('black')  # #00008B (darkblue)
        median.set_linewidth(5)
    
    ax.set_title('Retention rate by SWOT cycle', pad=24)  # was without pad
    ax.set_ylabel('Retention rate')
    ax.set_ylim(-0.01, 1.01)
    
    # ---- Dual x-axes: dates (bottom), cycle_id (top) ----
    # choose tick thinning for readability
    N = len(cycles)
    step = 1 if N <= 20 else (2 if N <= 50 else 5)
    show_idx = [i for i, _ in enumerate(cycles) if (i % step == 0)]
    
    # bottom: date labels
    ax.set_xlim(min(cycles) - 0.5, max(cycles) + 0.5)
    ax.set_xticks([cycles[i] for i in show_idx])
    date_labels = [
        (date_map.get(cycles[i]).strftime('%Y-%m-%d') if pd.notna(date_map.get(cycles[i], pd.NaT)) else '')
        for i in show_idx
    ]
    ...
    ax_top.set_xticks([cycles[i] for i in show_idx])
    ax_top.set_xticklabels([str(cycles[i]) for i in show_idx])
    ax_top.set_xlabel('SWOT cycle ID')
    plt.tight_layout()
    
    # --- 5) Save ---    
    plt.savefig(work_dir+'\Plots\Lake_retention_box_by_cycle_'+script_version+'.png')    
    # ==== 
...
if 'gauge_wse_bias_corrected' in df_lake_time_series.columns: #if there are at least lake with gauge data
    ...
    for lake_id in valid_lakes:
        ...
        x_rf = subset[['xovr_cal_q','xtrk_dist','dark_frac','quality_f','wse_std','wse_u','ice_clim_f']]
