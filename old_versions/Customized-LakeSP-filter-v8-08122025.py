...
from customized_functions import compute_rmse, compute_correlation, remove_tukey_outliers, \
        calibrate_heuristic_thresholds, apply_heuristic_thresholds, \
        filter_ice_outliers, convert_to_daily_series, signed_min_abs_residual, \
        filter_lowess, filter_savgol, filter_wavelet, filter_hampel, filter_spline, filter_median, filter_kalman  
...
Region       	  lake-count   Frequency	    Sources	                       Data-providers
  ...
  North America   589	       15 min to daily  ECCC, Quebec, USBR, USGS	   Merritt Harlan
  China	          38	       Daily	        Multiple authorities           Chunqiao Song
  India	          304	       Monthly	        NWIC, APWRIMS, Gujarat, CWC    Deep Shah;Huilin Gao
  West Africa	  2	           Hourly/finer	    In situ measure	               Manuela Grippa;Félix Girard;Laurent Kergoat
  Amazon          1	           Daily	        In situ measure (MISD)	       Ayan Fleischmann
  Ceará, Brazil   8	           Every 30 min	    In situ measure (Funceme)      Rafael Oliveira;Marielle Gosset;Eduardo Sávio Rodrigues Martins
  Other Brazil    43           Daily            To confirm                     Elyssa Collins;Augusto Getirana (originally 60 reservoirs)
  
Total: 1297			
Total unique (excluding duplicates): 1057 PLD lakes
"""
...
df_NA = pd.read_csv(work_dir + "/gauge_data/NA-from-Merritt-Harlan/gauge_data_08042025/NA_lake_gage_data_fulltimeseries.csv")
...
#test_lakeIDs = [4520076683] #,4550066742]
#test_lakeIDs = [6220306162] #[8221130683] #[4340276603]
...
# Remove any lakes that may have gauge data issues. 
lakes_to_exclude = [6240067663, 6340036253, 6420293613, 6420320993, 6420321093, 6420321803, 6420452243, 6420453523, \
                    6420468053, 6420468623, 6420469083, 6420478343, 6420513573, \
                    6420513583, 6420513623, 6420513653, 6420530103, 7421035423] # Other reservoirs in Brazil with gauge data issues (plus last NA).
...
for feature_id in test_lakeIDs: # test_lakeIDs contain a list of unique PLD lake IDs. 
    ...
    df.wse_std = df.wse_std.mask(df.wse_std == fill_float, np.nan) # Replace values in the wse_std column, where value = fill, with nan.
    df.xtrk_dist = df.xtrk_dist.mask(df.xtrk_dist == fill_float, np.nan) # Replace values in the xtrk_dist column, where value = fill, with nan.
    ...
    xtrk_dist_threshold_max = 0 #75000 is the valid max. Enter 0 if we opt to not use xtrk_dist as a threshold criterion. 
    xtrk_dist_threshold_min = 0
    ...
    # Recommendation: do not apply any of the two groupings (keep both False) for wse_std or wse_u to avoid over-rejection,
    #                 pass_id groupiong is theoretically needed for xtrk_dist as lake position varies in different passes. 
    df_heuristic_thresholds = calibrate_heuristic_thresholds(df_conservative,
                                       ...
                                       wse_std_threshold_minmax=[wse_std_threshold_min, wse_std_threshold_max],
                                       wse_u_threshold_minmax=[wse_u_threshold_min, wse_u_threshold_max],
                                       xtrk_dist_threshold_minmax=[xtrk_dist_threshold_min, xtrk_dist_threshold_max])
    ...
    while apply_low_pass_filter == 'yes' and (updated_length < initial_length) and (n_while < 40): # All conditions must be satisfied        
        ...
        exceeds_limit = (df_apply['datetime'].diff()) > pd.Timedelta(days=95) #92. 120 Check if any time difference exceeds ~3-4 months
    ...
    if apply_low_pass_filter == 'yes' and r2_filter == 'yes':
        ...
        while (updated_length < initial_length) and (n_while_r2 < 5): # A max of 5 iteration times to avoid over-rejection in round-2 filtering. 
            ...
            exceeds_limit = (df_eval['datetime'].diff()) > pd.Timedelta(days=95) #92. 120 Check if any time difference exceeds ~3-4 months
...
if unique_lakes: # If there is at least one lake with gauge data (i.e., rmse is not NaN)
    ...
    #test
    #lake_ids_var = subset.loc[subset["var_gauge_daily"] == 0, "lake_id"] #6420530103  
...
plt.savefig(work_dir+'\Plots\RF_feature_dependence.png') 
