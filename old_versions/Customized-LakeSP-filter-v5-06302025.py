...
Functions: Do not change the functions unless necessary. 
    Basic functions:
        ...
        compute_daily_variability: Computes std of daily-interpolated WSEs from SWOT and gauge data over their overlapping time range. 
...
def compute_daily_variability(df, gauge_df, 
                      time_col='datetime',
    ...
    """
    Computes standard deviations of daily-interpolated WSE time series
    from SWOT (raw and adjusted) and gauge data over their overlapping date range.
    ...
    return {
        'std_wse': wse_interp.std(),
        'std_wse_filtered': wse_filtered_interp.std(),
        'std_gauge': gauge_interp.std()
    }         
...
"""
# Initialize df_lake_time_series and df_lake_stats (the two major outputs) as empty dataframe
...
for feature_id in test_lakeIDs: # test_lakeIDs contain a list of unique PLD lake IDs. 
    ...
    Strategy: 
        - Rather than directly using the LakeSP summary quality flags for filtering, we leverage these flags to identify high-quality observations. 
        - From these selected observations, we empirically derive maximum thresholds for two key metrics:
    ...
    Notes:
        ...
        - The bitwise quality flag is excluded because it is only available for PIC2 and PID0 data (since October 2024).
    ...
    """
    # Define df_conservative as the subset of high-quality LakeSP observations, from which wse_std_threshold and wse_u_threshold are inferred.
    ...
    df_conservative = df_eval.query('xovr_cal_q < 1 & ice_clim_f < 2 & quality_f < 2') # Use "<" to retain fill values.   

    # Compute wse_std_threshold and wse_u_threshold
    # Apply conservative constraints on the thresholds to reduce the chance of including poor-quality observations.
    # !!! Note: These constraints are heuristic, lack strong theoretical justification, and may require future revision.!!!
    ...
    # Set up default upper limit: 3, 0.5
    ...
    if len(df_conservative) == 0: # If df_conservative is empty
        wse_std_threshold = wse_std_threshold_max # Default threshold
        wse_u_threshold = wse_u_threshold_max # Default threshold   
    else:
        wse_std_threshold = np.nanmax(df_conservative['wse_std'])   
        wse_u_threshold = np.nanmax(df_conservative['wse_u'])   
    if np.all(np.isnan(df_conservative['wse_std'])): # If wse_std is all nan
        wse_std_threshold = wse_std_threshold_max  
    if np.all(np.isnan(df_conservative['wse_u'])): # If wse_u are is all nan
        wse_u_threshold = wse_u_threshold_max  
    if wse_std_threshold > wse_std_threshold_max: 
       wse_std_threshold = wse_std_threshold_max
       #wse_std_threshold = np.max([df_conservative['wse_std'].quantile(0.9), 3]) # whichever is the larger one.     
    if wse_u_threshold > wse_u_threshold_max: 
       wse_u_threshold = wse_u_threshold_max
    elif wse_u_threshold < 0.1: #0.1 m overall consistent with the science requirement.
        wse_u_threshold = 0.1          
    
    # Check if we would like to execute both heuristic baseline filtering (Step 1) and low-pass filtering (Step 2) or just Step 1
    if apply_low_pass_filter == 'no': # Execute only the heuristic baseline filtering (Step 1)
        # Relax the wse_std threshold for ice-cover period to increase data availability so WSE phenology could be better represented.
        df_eval = df_eval[ (df_eval['wse_std'] <= wse_std_threshold) \
                         | ((df_eval['ice_clim_f']>0) & (df_eval['wse_std']<=wse_std_threshold_max)) ] 
        df_eval = df_eval[ df_eval['wse_u'] <= wse_u_threshold ] 
        # Note: no further criteria (e.g., 80% of the original time range) is applied above. 
    else: # execute both steps     
        # If preferred, further constrain df_eval before low-pass filtering. 
        if evaluating_at_full_data == 'no': # Evaluate outlier removal (z-score clipping) only on the selected observations below
            # Relax the wse_std threshold for ice-cover period to increase data availability so WSE phenology could be better represented.
            df_eval = df_eval[ (df_eval['wse_std'] <= wse_std_threshold) \
                             | ((df_eval['ice_clim_f']>0) & (df_eval['wse_std']<=wse_std_threshold_max)) ]            
            df_eval = df_eval[ df_eval['wse_u'] <= wse_u_threshold ]
    ...
    while apply_low_pass_filter == 'yes' and (updated_length < initial_length) and (n_while < 40): # All conditions must be satisfied
        ...
        # Select only good-quality observations for filter application to generate the smoothing curve.
        df_apply = df_eval.copy() # Initialize df_apply, which will be constrained to good observations only for curve smoothing. 
        # Relax the wse_std threshold for ice-cover period to increase data availability so WSE phenology could be better represented.
        df_apply = df_apply[ (df_apply['wse_std'] <= wse_std_threshold) \
                           | ((df_apply['ice_clim_f']>0) & (df_apply['wse_std']<=wse_std_threshold_max)) ] 
        # Note: based on my testing, this more lenient condition for ice-covered periods seems necessary. 
        df_apply = df_apply[ df_apply['wse_u'] <= wse_u_threshold ]
        ...
        # Remove bad observations flagged in PIC0 and PID0: specular_rining_bad, xovr_cal_bad, and low_coh_bad. 
        ...
        df_eval = remove_tukey_outliers(df_eval, col='wse', multiplier=3, lower_q=0.1, upper_q=0.9)
    ...
    if apply_low_pass_filter == 'yes' and recovering_observations == 'yes':        
        ...
        # Further restrict to observations meeting wse_std and wse_u thresholds (computed from round 1 filtering). No exception is made for ice-covered observations.
        df_good_quality = df_good_quality[(df_good_quality['wse_std'] < np.nanmin([wse_std_threshold,3])) & (df_good_quality['wse_u'] < wse_u_threshold)]  
    ...
    if apply_low_pass_filter == 'yes' and r2_filter == 'yes':
        ...
        while (updated_length < initial_length) and (n_while_r2 < 5): # A max of 5 iteration times to avoid over-rejection in round-2 filtering. 
            ...
            if maximum_residual_spread > 0.10: # An emprical threshold; no need to remove data if spread falls below this threshold.     
    ...
    if apply_low_pass_filter == 'yes' and intra_cycle_std > inter_cycle_std:
        ...
        df_eval = remove_tukey_outliers(df_eval, col='wse_adjusted', multiplier=3, lower_q=0.1, upper_q=0.9)
    ...
    if gauge_source is not None: # If this lake has gauge data (this works if df_eval is empty)
        ...
        var_daily = compute_daily_variability(df, gauge_df, 
                                            time_col='datetime', gauge_time_col='gauge_datetime',
                                            wse_col='wse', wse_filtered_col='wse_adjusted', gauge_wse_col='gauge_wse',
                                            interp_method='linear') 
        var_swot_daily = var_daily['std_wse_filtered'] #filtered daily variability during the full period
        var_gauge_daily = var_daily['std_gauge'] #gauge daily variability during the full period
        var_swot_daily_unfiltered = var_daily['std_wse'] #raw daily variability during the full period
        
        # Using CNES baseline 1
        var_daily_baseline1 = compute_daily_variability(df, gauge_df, 
                                            time_col='datetime', gauge_time_col='gauge_datetime',
                                            wse_col='wse', wse_filtered_col='wse_baseline1', gauge_wse_col='gauge_wse',
                                            interp_method='linear') 
        var_swot_daily_baseline1 = var_daily_baseline1['std_wse_filtered']
        
        # Using CNES baseline 2
        var_daily_baseline2 = compute_daily_variability(df, gauge_df, 
                                            time_col='datetime', gauge_time_col='gauge_datetime',
                                            wse_col='wse', wse_filtered_col='wse_baseline2', gauge_wse_col='gauge_wse',
                                            interp_method='linear')  
        var_swot_daily_baseline2 = var_daily_baseline2['std_wse_filtered']
    ...
    # Concatenate df_lake_time_series by this df, and df_lake_stats by df_this_lake_stats
    ...
    if gauge_source is None: # No gauge data
        textstr = f'wse_std <= {wse_std_threshold:.3f}\nwse_u <= {wse_u_threshold:.3f}'
        props = dict(boxstyle='round', facecolor='lightgrey', alpha=0.8)
        ax.text(0.05, 0.95, textstr, transform=ax.transAxes, fontsize=10, verticalalignment='top', bbox=props)
    else: # With gauge data, also show part of the validation statistics
        textstr = f'wse_std <= {wse_std_threshold:.3f}\nwse_u <= {wse_u_threshold:.3f}\nrmse_unfiltered = {rmse_unfiltered:.3f}\nrmse = {rmse:.3f}'
        props = dict(boxstyle='round', facecolor='lightgrey', alpha=0.8)
        ax.text(0.05, 0.95, textstr, transform=ax.transAxes, fontsize=10, verticalalignment='top', bbox=props)
    #ax.legend()  
