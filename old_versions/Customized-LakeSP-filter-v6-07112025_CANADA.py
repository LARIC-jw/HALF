# -*- coding: utf-8 -*-
"""
Created on Thu Jul 10 05:35:30 2025
...
df_Canada_GNSS = pd.read_csv(work_dir+'/gauge_data/Canada-Melanie-Trudel/BD_Canada_GNSS.csv', sep=',', encoding='iso-8859-2')
# Gage with unknown datum
df_Canada_UN = pd.read_csv(work_dir+'/gauge_data/Canada-Melanie-Trudel/BD_Canada_unknown.csv', sep=',', encoding='iso-8859-2')
df_Canada = pd.concat([df_Canada_GNSS, df_Canada_UN ]).reset_index(drop=True)                         
#df_Canada=df_Canada_GNSS.head(18000).reset_index(drop=True) 
...
for n in range(len(unique_pld_ids)):
    new_row = pd.DataFrame([{
        ...
        'gauge_source': 'Canada',
        'gauge_dir': work_dir+'/gauge_data/Canada-Melanie-Trudel/',
...
# # remove canadian lake
# df_NA = df_NA[(df_NA['agency'] !='Quebec')]
# df_NA = df_NA[(df_NA['agency'] !='ECCC')]
...
#        df_Hydrocron.to_csv(work_dir + '\\df_Hydrocron2.csv', index=False)
    ...
    if gauge_source == 'Canada': # Quebec lakes
        ...
        # Filter for best gage_id with the closest geoid 
        gauge_df = gauge_df[gauge_df["OBS_ID"] == best_gage_id]
#        gauge_df = gauge_df[gauge_df["OBS_ID"] == best_gage_id][["OBS_WSE", "OBS_ID"]]
        # same time than SWOT
...
#MT        # compute bias between gauge and SWOT. This can be cause by unknown levelling of 
           #by the difference between the average geoid of the lake an the geoid at the station
           # median of difference on ice-free observations is used to unbias
        bias_swot_gauge = gauge_df['gauge_wse'][gauge_df['ice_clim_f']<1] -gauge_df['wse'][gauge_df['ice_clim_f']<1]    
        ax.plot(gauge_df['gauge_datetime'], gauge_df['gauge_wse']-np.nanmedian(bias_swot_gauge), \
                label='gauge', color='green', marker = 'o', markersize=8, linestyle='--') # Include an approximate datum adjustment
#        ax.plot(gauge_df['gauge_datetime'], gauge_df['gauge_wse']-(gauge_df['gauge_wse'].mean()-df['wse'].mean()), \
#                label='gauge', color='green', marker = 'o', markersize=8, linestyle='--') # Include an approximate datum adjustment
...
#MT     here again, I think it is better to unbias with the median of ice-free observations   
        # Compute RMSE
...
#MT        wse_adjusted_mean = np.nanmean(subset['wse_adjusted'])
#MT        gauge_mean = np.nanmean(subset['gauge_wse'])
#MT        rmse_unfiltered = compute_rmse(df['wse']-wse_adjusted_mean, df['gauge_wse']-gauge_mean)
        rmse_unfiltered = compute_rmse(df['wse'], df['gauge_wse']-np.nanmedian(bias_swot_gauge))
...
#        rmse = compute_rmse(df['wse_adjusted']-wse_adjusted_mean, df['gauge_wse']-gauge_mean)
        rmse = compute_rmse(df['wse_adjusted'], df['gauge_wse']-np.nanmedian(bias_swot_gauge))
        # Using CNES baseline 1:
#        rmse_baseline1 = compute_rmse(df['wse_baseline1']-wse_adjusted_mean, df['gauge_wse']-gauge_mean)
        rmse_baseline1 = compute_rmse(df['wse_baseline1'], df['gauge_wse']-np.nanmedian(bias_swot_gauge))
        # Using CNES baseline 2:
#        rmse_baseline2 = compute_rmse(df['wse_baseline2']-wse_adjusted_mean, df['gauge_wse']-gauge_mean)
        rmse_baseline2 = compute_rmse(df['wse_baseline2'], df['gauge_wse']-np.nanmedian(bias_swot_gauge))
...
if unique_lakes: 
    ...
    gauge_wse_anomalies = []
#MT variable to compute the importance feature with random forest
    SWOT_error = pd.Series(dtype='int64')
    X_RF=pd.Series(dtype='int64')
...
#MT        # Compute |wse error| using median to unbias 
        
        swot_error = np.abs(df['wse'] - (df['gauge_wse']-np.nanmedian(bias_swot_gauge)))
        SWOT_error = pd.concat([SWOT_error,  swot_error], ignore_index=True)
        x_rf = df[['xovr_cal_q','xtrk_dist','quality_f','wse_std','wse_u','ice_clim_f']]
...
#MT compute RF to determine importance feature for |swot_error|
...
# Using all data, the most important feature is xtrk_dist, probably related
# to specular ringing or other errors near the nadir 
# It is interreting to see the importance features for large error and for small error

X_RF.xtrk_dist=np.abs(X_RF.xtrk_dist)
SWOT_error_ALL = SWOT_error
X_RF_All = X_RF

X_RF = X_RF[SWOT_error_ALL<30]
SWOT_error=SWOT_error[SWOT_error_ALL<30]
...
plt.title('Importance feature (Random Forest)')
plt.gca().invert_yaxis()
plt.savefig(work_dir+'\Plots\RF.png') 
